(async () => {
  ...
  const caseUrlPasses = (u) => {
    ...
    if (/\/(articles|solutions)(\/|$)/.test(url.pathname)) return false;
  ...
  const passed = fixedChecks.every((c) => c.status === c.expected) && casesPassed && sitemapPassed;
  console.log(JSON.stringify({ passed, checks: [...fixedChecks, ...caseChecks], sitemapPassed, caseState: contract.state, notificationSent: false, inquirySubmitted: false }));
