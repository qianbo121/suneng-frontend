...
`backup.sh` 把数据库导出与附件打包写进 `/data/backup`，保留 7 天。它与备份目标同机同盘，且目前不校验产物、失败也不告警——换机或重装时必须重新装上这条 cron，并在恢复前先验证归档可读。发布前的隔离恢复演练用的是另一套 `ops/releases/data_backup.py`（capture + restore），不要与这条 cron 混为一谈。
