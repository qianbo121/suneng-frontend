...
const implementationStages = [
  ...
  },
  {
    ...
    items: [
      ...
      '制造周期受供货边界、材料交期和验收安排影响，以双方确认的排程为准。',
    ],
  },
  {
    title: '阶段 3：现场安装与调试',
...
const effectRows: FactItem[] = [
  ['收益结论', '节能收益需按项目工况、可比数据与双方约定的核算边界确认。'],
  ['核算方法', '基于可比工况下的燃料消耗、能源价格、有效产量、运行制度和改造边界进行项目级测算。'],
  ['吨钢降本构成', '可从燃料结构、多级烟气余热回收、控温系统与风机运行等维度拆解；各项贡献须以项目数据核算。'],
  ['适用边界', '本页不披露未经授权的精确收益、吨钢降本或测算产量，也不将单一项目测算外推为通用承诺。'],
];

const otherEffects = [
  {
    title: '排放验收边界',
    text: '燃烧系统按项目所在地排放要求设计。是否达标必须对应适用标准、测试条件和项目边界，并以有资质第三方检测或正式验收报告为准；本页不以设计目标代替检测结论。',
  },
  {
    title: '控温精度评估',
...
const caseJsonLd = {
  ...
  description:
    '苏能工业炉为某不锈钢深加工企业提供连续退洗线节能改造方案，介绍燃料结构适配、多级烟气余热回收、控温系统升级、分阶段实施与节能收益核算方法。',
    ...
    <div className="bg-white text-[#101828]">
        ...
        <div className="relative mx-auto max-w-[1180px] px-5 py-14 lg:px-8 lg:py-20">
          ...
          <div className="mt-10 max-w-[980px]">
            ...
            <p className="mt-7 max-w-[940px] rounded-[8px] bg-white/10 p-5 text-[15px] leading-[1.9] text-white/78">
              ...
              本页公开不涉及客户敏感信息的技术路径；具体项目规模与效果以经授权的项目资料、现场诊断和正式验收文件为准。
            </p>
      ...
      <Section id="results" eyebrow="Results" title="五、改造后效果与数据拆解">
        ...
        <article className="mt-7 rounded-[8px] border border-[#d6e0ec] bg-[#f8fafc] p-6">
          ...
          <p className="mt-4 text-[15px] leading-[1.9] text-[#344054]">
            本页不披露未经授权的精确年收益、吨钢降本与测算产量，也不将内部测算值作为客户实际经营数据或对其他项目的效果承诺。
          </p>
