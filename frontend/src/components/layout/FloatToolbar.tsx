    ...
    <>
      <div className="fixed bottom-8 right-6 z-40 hidden flex-col transition-opacity duration-200 [body.mobile-nav-open_&]:hidden xl:flex">
        {items.map((item, index) => {
          const sharedClassName =
            'group flex h-[92px] w-[72px] flex-col items-center justify-center gap-[10px] border border-white/10 bg-[#0b1f36] text-white shadow-[0_14px_28px_rgba(0,0,0,0.18)] transition-colors duration-200 hover:bg-[var(--color-interactive)]';
        ...
        {items.map((item) => {
          if (item.type === 'top') {
            ...
            return (
              ...
              >
                <TopIcon className="text-lg text-[var(--color-interactive)]" aria-hidden="true" />
          ...
          if (item.type === 'wechat') {
            return (
              <button
                ...
                className="flex min-h-[64px] flex-col items-center justify-center gap-1 text-[11px] text-neutral-700"
              >
                <FaWeixin className="text-lg text-[var(--color-interactive)]" aria-hidden="true" />
                <span>{item.label}</span>
              </button>
            );
          }
      ...
      {wechatOpen ? (
        <div
          ...
          tabIndex={-1}
          className="fixed inset-0 z-[80] flex items-center justify-center overscroll-contain bg-black/45 px-4"
        >
          <div className="w-full max-w-[280px] rounded-[10px] bg-white p-6 text-center shadow-[0_20px_60px_rgba(0,0,0,0.26)]">
            ...
            <button
              ...
              className="mt-5 h-[38px] min-w-[120px] rounded-[4px] bg-[var(--color-interactive)] px-6 text-[14px] text-white transition-colors hover:bg-[var(--color-interactive-strong)]"
            >
