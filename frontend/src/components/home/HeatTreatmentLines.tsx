...
export const heatTreatmentLines: HeatTreatmentLineCard[] = [
  {
    title: { zh: '托辊型网带式电阻炉生产线', en: 'Mesh-Belt Annealing & Tempering Line' },
    desc: { zh: '连续退火回火，托辊输送与多温区控制', en: 'Annealing or post-quench tempering, with a roller-supported mesh belt and zoned temperature control.' },
    image: '/images/products/roller-mesh-belt-line/gallery/line-01.jpg',
    slug: 'roller-mesh-belt-line',
  },
  {
    title: { zh: '铜丝自动化退火生产线', en: 'Continuous Copper-Wire Annealing Line' },
    desc: { zh: '铜丝连续退火，放收线一体自动化生产', en: 'Continuous wire annealing with coordinated pay-off, cooling and take-up.' },
    image: '/images/products/copper-wire-annealing-line/gallery/line-01.jpg',
    slug: 'copper-wire-annealing-line',
  },
  {
    title: { zh: '退火固溶生产线', en: 'Continuous Annealing & Solution-Treatment Line' },
    desc: { zh: '退火固溶多区控温，按材料与速度评估配置', en: 'Process zones, line speed and cooling configured for the material and treatment route.' },
    image: '/images/products/annealing-solution-line/gallery/line-01.jpg',
    slug: 'annealing-solution-line',
  },
];

const singleFurnaceCards: ProductCenterCard[] = homeSingleFurnaces.map((item, index) => ({
  id: index + 1,
  en: item.englishName,
  zh: item.name,
  feature: { zh: item.description, en: item.descriptionEn },
  image: item.image,
  slug: item.id,
}));
...
const featureItems: FeatureItem[] = [
  ...
  {
    title: { zh: '按节拍配置', en: 'Throughput planning' },
...
export function HeatTreatmentLines({ locale, categories }: HeatTreatmentLinesProps) {
  ...
  const systemHref = locale === 'zh' ? '/zh/products#continuous-furnace-title' : '/en/products#continuous-lines';
        ...
        <div className="overflow-hidden rounded-[6px] border border-[#dfe5ee] bg-white">
          <div className="grid gap-8 p-5 md:p-7 lg:grid-cols-2 lg:gap-10 lg:p-9">
            <div className="flex flex-col justify-center">
              <h2 className="text-[30px] font-semibold leading-[1.18] text-[#101828] md:text-[38px] lg:text-[40px] xl:text-[44px]">
              ...
              <div className="mt-9 grid grid-cols-2 border-t border-[#d8dee8] pt-7 sm:grid-cols-4">
                {featureItems.map((item) => (
                  <div
                    key={item.title.zh}
                    className="min-w-0 border-r border-[#e1e6ee] px-4 py-2 text-[#101828] first:pl-0 last:border-r-0"
                  >
                    <p className="text-[16px] font-semibold leading-tight">{item.title[locale]}</p>
                    <p className="mt-2 text-[13px] leading-[1.5] text-[#526277]">{item.desc[locale]}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="mt-6 lg:mt-7">
            <div className="grid grid-cols-2 border-l border-t border-[#ebebeb] lg:grid-cols-4">
              {productCenterCards.map((item) => (
                ...
                >
                  <p className="text-[10px] font-medium uppercase tracking-[0.16em] text-[#526277]">
                    {item.en}
                  </p>
                  <h4 className="mt-2 text-[17px] font-semibold leading-[1.4] text-[#1f1f1f] lg:text-[18px]">
