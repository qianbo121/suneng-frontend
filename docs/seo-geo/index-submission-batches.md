...
1. `F01` 三条线 / 1250 mm 与 `F05` 合同节点已于 2026-07-31 完成原件核查，可按事实台账边界使用；
2. `F02` 7,644 万元/年、`F03` 63.7 元/吨、`F04` 120 万吨/年已找到 2015 年技术方案测算出处，但仍须唐工确认其计算口径和适用边界；
3. F02–F04 如继续展示，必须明确写成“技术方案测算值 / 测算假设”，不能写成客户实际收益、实际单位成本或实际产量；
4. `F06` 本机未检出第三方检测或正式验收报告；取得可核验报告前，只能使用“以有资质第三方检测或正式验收为准”的范围表达，不能发布“已达标”；
5. 青山页面按上述边界完成实质内容修正，且页面内容、结构化数据与事实台账一致。
...
1. `https://www.jssngyl.cn/zh/solutions/rechuli-lu-luchen-fanxin`
2. `https://www.jssngyl.cn/zh/solutions/rechuli-lu-dian-gai-ran-yure-huishou`
3. `https://www.jssngyl.cn/zh/solutions/rechuli-lu-kongzhi-xitong-shengji`
4. `https://www.jssngyl.cn/zh/solutions/rechuli-lu-tingchan-chongqi-banqian-fuchan`

执行结果：

- 4/4 页面已上线并返回 200；
- 4/4 已进入 sitemap；
- IndexNow 接口返回 `HTTP 200`；
- 百度 API 上一轮响应仅剩 1 条额度；本轮优先尝试提交电改燃 / 燃改电 / 余热回收页，但本地与服务器请求均未取得百度成功响应，Chrome 会话也未登录搜索资源平台，因此未记为成功、未推定额度已消耗；4 页进入下一次可验证提交批次；
- 青山案例不在本批；
- 接口接收不等于抓取、收录或引用，后续纳入 28 页 Nginx 日志监测。
